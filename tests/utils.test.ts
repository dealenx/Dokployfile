...
import { generateDokployPayload, decodeDokployPayload } from '../app/utils/payload';
...
`;
    ...
    expect(results.some(r => r.message.includes('container_name'))).toBe(true);
  });
...
`;
    const results = lintComposeFile(yaml);
    expect(results.some(r => r.message.includes('host-port mapping'))).toBe(true);
  });
});
